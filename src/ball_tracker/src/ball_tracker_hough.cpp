...
int main(int argc, char** argv){
  ...
  const int factor = 21;
  const int range = 12;
  const int satMin = 110, valMin = 10;
  ...
  createTrackbar("cannyThreshold", windowName, &cannyThreshold,255);
  createTrackbar("accumulatorThreshold", windowName, &accumulatorThreshold, 255);
  ...
  while(ros::ok()){
    ...
    //erode(imgFinal, imgFinal, element);
    dilate(imgFinal, imgFinal, element2);
    dilate(imgFinal, imgFinal, element2);
    //erode(imgFinal, imgFinal, element);

    //cvtColor( frame, map_x, COLOR_BGR2GRAY );
    GaussianBlur( imgFinal, imgFinal, Size(9, 9), 2, 2 );
    // Apply the Hough Transform to find the circles
    HoughCircles(imgFinal, circles, CV_HOUGH_GRADIENT, 1, imgFinal.rows/8,
      cannyThreshold, accumulatorThreshold, 10, 0);
    ...
    for( size_t i = 0; i < circles.size(); i++ )
    {
         ...
         circle(frame, center, radius, Scalar(0,255,0), 9, 8, 0);
    }
